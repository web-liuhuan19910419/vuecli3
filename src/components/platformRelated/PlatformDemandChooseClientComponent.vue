          ...
          :style="platformStyleObject" @click.stop="ignoreEmitHidden">
            <div class="platform-content">
              ...
              <div class="platform-body">
                ...
                <div class="sep-line-layout"></div>
                <ul class="ul-layout">
                  <li v-for="(supplierItem, index) in onSearch(keyword)" :key="supplierItem.uid">
...
export default {
  ...
  methods: {
    ...
    onSearch (key) {
      let newList = []
      this.$store.state.supplierAndPurchaseModule.purchaseOrderCompanyCodesData.forEach(element => {
        if (element.company_code.indexOf(key) !== -1) {
    ...
    onIndexItem (index) {
      console.log(this.onSearch(this.keyword)[index].company_code)
      this.onSearch(this.keyword)[index].checked = !this.onSearch(this.keyword)[index].checked
    },
    onTotalSwitchChecked () {
      let isTotalChecked = true
      if (this.onSearch(this.keyword).length === 0) {
        isTotalChecked = false
      }
      for (let index = 0; index < this.onSearch(this.keyword).length; index++) {
        let element = this.onSearch(this.keyword)[index]
      ...
      if (isTotalChecked === true) {
        ...
        for (let index = 0; index < this.onSearch(this.keyword).length; index++) {
          const element = this.onSearch(this.keyword)[index]
    ...
    // 多选radio的状态
    checkedSrc (index) {
      if (this.onSearch(this.keyword)[index].checked === null) {
        return ''
      } else if (this.onSearch(this.keyword)[index].checked === true) {
        return require('../../assets/images/radio_checked.png')
      } else if (this.onSearch(this.keyword)[index].checked === false) {
