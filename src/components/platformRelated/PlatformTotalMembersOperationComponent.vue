          ...
          :style="platformStyleObject" @click.stop="ignoreEmitHidden">
            <div class="platform-content">
              ...
              <div class="platform-body">
                ...
                <div class="sep-line-layout"></div>
                <ul class="ul-layout">
                  <li v-for="(supplierItem, index) in this.$store.state.supplierAndPurchaseModule.totalSupplierData" :key="supplierItem.id">
...
export default {
  ...
  data () {
    return {
      ...
      platformHeader: '从全部供应商中选取'
    }
  },
  methods: {
    onIndexItem (index) {
      this.$store.state.supplierAndPurchaseModule.totalSupplierData[index].checked = !this.$store.state.supplierAndPurchaseModule.totalSupplierData[index].checked
    },
    onTotalSwitchChecked () {
      let isTotalChecked = true
      if (this.$store.state.supplierAndPurchaseModule.totalSupplierData.length === 0) {
        isTotalChecked = false
      }
      for (let index = 0; index < this.$store.state.supplierAndPurchaseModule.totalSupplierData.length; index++) {
        let element = this.$store.state.supplierAndPurchaseModule.totalSupplierData[index]
    ...
    // 多选radio的状态
    checkedSrc (index) {
      if (this.$store.state.supplierAndPurchaseModule.totalSupplierData[index].checked === null) {
        return ''
      } else if (this.$store.state.supplierAndPurchaseModule.totalSupplierData[index].checked === true) {
        return require('../../assets/images/radio_checked.png')
      } else if (this.$store.state.supplierAndPurchaseModule.totalSupplierData[index].checked === false) {
