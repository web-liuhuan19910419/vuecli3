...
export default {
  props: {
    ...
    isCheckAllToCheckAllProp: false,
    gridDataListProp: []
  },
  data () {
    return {
      isCheckAllLocal: false,
      gridDataList: []
    }
  },
  methods: {
    checkAllStatus () {
      ...
      for (let index = 0; index < this.gridDataList.length; index++) {
        let element = this.gridDataList[index]
  ...
  beforeMount () {
    this.gridDataList = this.gridDataListProp
  },
  watch: {
    // 观察isCheckAllToCheckAllProp变化，调整CheckAllComponent全选或不全选
    'isCheckAllToCheckAllProp' (to, from) {
      this.isCheckAllLocal = to
    },
    'gridDataListProp' (to, from) {
    }
  }
}
</script>
