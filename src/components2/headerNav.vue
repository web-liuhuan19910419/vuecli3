<template>
  <ul class="content-bg-layout">
    <li v-for="(item,index) of quickLinkListData" :key="index" @click="clickCurrntIndex(index)" :class="[defaultActiveIndex === index ? 'item-active-styles' : 'item-default-styles']">{{$t(item.name)}}</li>
  </ul>
</template>
<script>
  export default {
    ...
    watch: {
      'quickLinkListProp' (to, from) {
        if (to !== undefined) {
          this.quickLinkListData = to
        }
      },
      'defaultActiveIndexProp' (to, from) {
        if (to !== undefined) {
